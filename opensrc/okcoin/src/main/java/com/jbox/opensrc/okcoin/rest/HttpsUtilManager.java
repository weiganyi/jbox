package com.jbox.opensrc.okcoin.rest;
 ...
 *
 */
public class HttpsUtilManager {
	private static final Integer CONNECT_TIMEOUT = 3*1000;

	private static HttpsUtilManager instance = new HttpsUtilManager();
	...
	private HttpsUtilManager() {
		client = HttpClients.custom().setSSLSocketFactory(createSSLConnSocketFactory2()).setConnectionManager(cm).setKeepAliveStrategy(keepAliveStrat).build();
	}
	...
	public static HttpsUtilManager getInstance() {
		return instance;
	}
	...
	public String requestHttpGet(String url_prex,String url,String param) throws HttpException, IOException{
		...
		method.addHeader("Host", "www.okcoin.cn");
